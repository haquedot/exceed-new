...
function submitForm() {
    ...
    getCartCount()
    window.location = "./ExceedCart.php";
}
...
function getClassList() {
    ...
    if (!registration) {
        window.location = './ExceedRegisteration.php'
    }
            ...
            $.each(courses, function(courseInd, courseElement) {
                ...
                $.each(classes, function(i, element) {
                    ...
                    if (element.allowToSelectClass) {
                        //<button type="button" class="btn-primary rounded" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalCenter">Add to cart</button>
                        actionString = `<button type='button' class='btn-primary rounded' onclick='openStudentModal("${element.classId}", "${element.minimumAge}")'>Add to cart</button>`
                    } else {
                    ...
                    $('#classListTable2').append(`<div class="row border border-grey rounded mx-1 p-0">
            <div class="col-lg-4 class-content class-content-1 rounded-top">
                ${element.session}
            </div>
            <div class="col-lg-2 class-content class-content-2">
                <p>
                    <span>Minimum Age:</span> ${element.minimumAge}
                </p>
            </div>
            <div class="col-lg-2 class-content class-content-3">
                <p>
                    <span>Date:</span> ${element.dates}
                </p>
            </div>
            <div class="col-lg-2 class-content class-content-4">
                <p>
                    <span>Time:</span> ${element.time}
                </p>
            </div>
            <div class="col-lg-2 class-content class-content-5 p-2">
                <div class="container-fluid p-0 class-content-5">
                   ${actionString} 
                </div>
            </div>
        </div>`);
